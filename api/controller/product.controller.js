...
async function addProduct(req, res, next) {
    try {
      const { name, quantity, price, description, category, status, isFeatured } = req.body;
      if (!name || !price || !category || !quantity || !category || !status) {
  ...
  async function editProduct(req, res, next) {
    try {
      ...
      const { name, price, quantity, description, category, isFeatured } = req.body;
      ...
      if (existing) {
        return res.status(400).json({ message: 'Slug này đã tồn tại cho sản phẩm khác' });
      }
...
module.exports = {
    addProduct,editProduct,changeStatusProduct,getAllPro,getProductById
};
